...
public class Connect {
    ...
    public static Connection openConnection() {
        try {
            ...
            conn = DriverManager.getConnection("jdbc:mysql://localhost/video_rating?user='root'&password=");
...
//txtfirstname, txtmiddlename, txtlastname, txtemailid, txtmobile, cmbgender, txtaddress, txtpassword, "True", "user"
    ...
    public static void updateField(String table, String field, String value, String matching_field, String match) {
      ...
      public static int saveGame(String gname, String gdesc,String gimage,String gcategory,String videoname) {
        openConnection();
        int i = 0;

        try {

            String sql = "insert into tblvideos (gname,gdesc,gimage,gcategory,rdate,videoname)values(?,?,?,?,?,?)";
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, gname);
            statement.setString(2, gdesc);
            statement.setString(3, gimage);
            statement.setString(4, gcategory);
            statement.setString(5, getDate());
            statement.setString(6,videoname);
            i = statement.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
